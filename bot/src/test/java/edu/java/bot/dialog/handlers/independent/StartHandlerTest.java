...
public class StartHandlerTest {
    ...
    private static final String CORRECT_COMMAND = "/start";
    @Mock
    ...
    @Autowired
    private UpdateHandler startHandler;
    @Autowired
    ...
    void givenCorrectUpdate_thenCorrectHandling() {
        ...
        var responses = startHandler.handle(update, USER_DATA);
    ...
    void givenCorrectUpdate_thenCorrectUserStateTransition() {
        Mockito.when(update.message()).thenReturn(message);
        Mockito.when(message.text()).thenReturn(CORRECT_COMMAND);
        startHandler.handle(update, USER_DATA);
    ...
    @Test
    void givenCorrectUpdateWithUnregisteredUser_thenCorrectHandling() {
        Mockito.when(update.message()).thenReturn(message);
        Mockito.when(message.text()).thenReturn(CORRECT_COMMAND);
